...
class DeformableConv2dLayer(tf.keras.layers.Conv2D):
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        # some length
        batch_size = int(inputs.get_shape()[0]) if inputs.get_shape()[0] is not None else 1 # TODO: May be has a bug?
        ...
        y, x = [tf.tile(i, [batch_size, 1, 1, 1, self.num_deformable_group])
                for i in [y, x]]
        ...
        p0, p1, p2, p3 = [DeformableConv2dLayer._get_pixel_values_at_point(
        ...
        pixels = tf.reshape(pixels, [batch_size, out_h * filter_h,
        ...
        out = tf.reshape(
            out, [batch_size, out_h, out_w, self.filters, channel_in])
...
if __name__ == '__main__':
    ...
    y = DeformableConv2dLayer(filters=64, 
                              kernel_size=5,
                              strides=2,
                              padding='same',
                              activation='relu',
